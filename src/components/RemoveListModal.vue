    ...
    <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
      ...
      <div class="MODAL_PANEL relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg ease-out duration-300 opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" :class="{'opacity-100 translate-y-0 sm:scale-100': show}">
            ...
            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
              ...
              <div class="mt-2">
                ...
                <div style="white-space:pre-wrap;overflow-wrap:break-word">
                  <slot></slot>
                  <label for="listName" class="transition-colors duration-500 text-gray-900 dark:text-gray-200" :style="{whiteSpace: 'pre-wrap', overflowWrap: 'break-word'}">Do you really want to remove List: {{ props.listName }}</label>
        ...
        <div class="bg-gray-50 dark:bg-gray-900 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
          <button type="button" @click="() => { emit('accept'); handleAccept(); }" class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 sm:ml-3 sm:w-auto">{{ props.okTitle }}</button>
...
const handleAccept = () => {
  let createOut:any = deleteTodoList(props.listName); 
  if(createOut.error){
